...
⚙️ **升級動作**:`git pull` 後,**舊 config.json 缺 `thread_watch` 鍵會用預設值頂上並在整合健檢提示一次**,不會靜默失效;想改參數自行加進 config.json,或跟秘書說「thread 只追 10 串」「觀察名單留一週就好」。無新 scope。**額度增量**:層 ① 20 串 × 每 30 分一輪約 +40~70k tokens/天,層 ② 50 串 × 一天 2 次約 +15k/天(多數查詢回 0 則新訊息);Pro 方案建議 `max_threads` 8 / `observe_max` 20
