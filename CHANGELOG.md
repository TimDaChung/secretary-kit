...
- **修正連結指到錯位置**（Tim 回報）：thread 內訊息的連結少了 `?thread_ts=&cid=`，Slack 不會開那一串、只把人丟到頻道。成因是用 `channel + ts` 自己拼網址——**串根 ts 與訊息 ts 常常差很遠**（實測：訊息 `1790065718.501899` 的串根是 `1788847271.645089`），拼出來的網址必定指錯
  - 連結規則改寫成四條：①一律存 API 回傳的 permalink **原樣、含 query string 不得裁切**；②**禁止用 channel+ts 自己拼**；③只有 channel+ts 時呼叫 `chat.getPermalink` 補；④**禁止用純頻道連結充數**（`/archives/<channel_id>` 點了落在頻道最新訊息，比沒連結更誤導）——真的沒有單一來源訊息（自記 notes、跨多則的承諾彙整）就**不附連結**，改寫「(來源:與 X 的 DM,無單一訊息)」
- **`watched_threads[]` / `observed_threads[]` 加 `link` 欄**：入列當下就存該串 permalink。原本只存 `channel` + `last_seen_ts`，要報這串時無從產生正確連結

⚙️ **升級動作**：`git pull` 後生效。**既有 state.json 的舊項目不會自動修**——舊項的 `link` 仍是舊格式，下一輪新建的項目才會是正確連結；在意的話請秘書「重掃」或等舊項銷帳自然汰換。無新 scope、無額度增量
