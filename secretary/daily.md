...
**名單上限** `config.thread_watch.observe_max`(預設 50):滿了擠掉**最久沒人講話的**(`last_seen_ts` 最舊,與 SKILL.md 1.4 層 ① 同一排序鍵),擠掉的不另行通知。層 ① 溢出降級下來的串也併入本名單一起排序。理由:14 天窗口 × 每天新進幾串會無聲累積,兩大輪各查一次 = 每串每天 2 次查詢,沒上限會讓成本失控。
