...
- **委派型**:含「秘書」+任意委派動詞(分析/查/整理/處理/準備/擬/追/提醒我/幫我 X 等,不限這些字)→ **記下不執行**:存 notes 標 `type: task`+建 P1 追蹤項「你交辦秘書:<一句摘要>(含原訊息連結)」,bot 回「📋 收到交辦:XXX——在終端跟我說一聲就開工」。不擅自執行的原因:委派可能涉及對外動作或大量工作,等使用者在終端確認;使用者說「做 N」或口頭指示 → 主 session 執行
